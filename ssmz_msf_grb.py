	...
	for k in range(0,len(c_nodos)):
		for p in c_nodos[k]:
			if p!=N:
				...
				f.write("\\fill[fill=col"+str(k)+",opacity=0.55]("+str(j)+","+str(m-(i+1))+") rectangle ( " +str(j+1) +","+str(m-i)+");\n")
	...
	for k in range(0,len(v_list)):
		for q in v_list[k]:
			if q!=N:
				i=int(q/n)
				j=q % n
				f.write("\\node[fill=col"+str(k)+",text=black, font=\\fontsize{6}{6},opacity=0.55,text opacity=1,circle,minimum size=0.8cm](v"+ str(q) +") at ("+str(j+0.5)+","+str(m-i-0.5)+"){$"+str(q+1)+"$};\n")
				#f.write("\\node[draw,thick,color=black,text=black, font=\\fontsize{6}{6},opacity=0.8,text opacity=1,circle,minimum size=0.8cm](v"+ str(q) +") at ("+str(j+0.5)+","+str(m-i-0.5)+"){$"+str(q+1)+"$};\n")

	i=int(N/n)
	j=N % n
	f.write("\\node[draw,font=\\fontsize{8}{8},opacity=0.8,circle,minimum size=0.4cm](v"+ str(N) +") at ("+str(j+0.5)+","+str(m-i-0.5)+"){$"+str(N)+"$};\n")
	#for e in G.edges():
	#	if e[0]!=N and e[1]!=N:
	#		f.write("\\draw[-latex,bend left=10,color=black,thick,opacity=1.0](v"+str(e[0])+") edge (v"+str(e[1])+");\n")
	#	else:
	#		f.write("\\draw[-latex,bend left=10,color=black,thick,opacity=0.5](v"+str(e[0])+") edge (v"+str(e[1])+");\n")

     
	for k in range(0,len(e_list)):
		for q in e_list[k]:
			#if q[0]!=N and q[1]!=N:
			f.write("\\draw[-latex,bend left=0,color=col"+str(k)+",thick,opacity=1.0](v"+str(q[0])+") edge (v"+str(q[1])+");\n")
	f.write("\\end{tikzpicture}\n")
	f.write("\\end{document}\n")
	f.close()
	os.system("pdflatex --interaction=batchmode " + nombre+".tex")	
...
for u in G.nodes():
	for k in range(0,K):
		...
		aux[u,k]=mod.addVar(vtype=grb.GRB.CONTINUOUS,name="aux_"+str(u)+"_"+str(k))
...
for k in range(0,K):
   ...
   z[k]=mod.addVar(vtype=grb.GRB.CONTINUOUS,name="z_"+str(k))
...
for e in G.edges():
	for k in range(0,K):
		...
		mod.addConstr(y[e[0],e[1],k] + y[e[1],e[0],k]  <= prod[e[0],e[1],k] ,"anticolor_"+str(e[0])+"_"+str(e[1])+"_"+str(k))
		mod.addConstr(prod[e[0],e[1],k] <= x[e[0],k])
		mod.addConstr(prod[e[0],e[1],k] <= x[e[1],k])
		mod.addConstr( prod[e[0],e[1],k] >= x[e[0],k]+x[e[1],k] - 1 )
...
for u in G.nodes():
	if u!=K:
		for k in range(0,K):
			mod.addConstr( grb.quicksum([y[u,q,k] for q in G.neighbors(u)]) == x[u,k] , "El_vertice_"+str(u)+"_tiene_flujo_si_esta_con_color_"+str(k))
...
#grb.quicksum( [x[u,k] for u in G.nodes() if u!=K ])
...
for k in range(0,m*n):
  # mod.addConstr(media[k]*z[k] == grb.quicksum( [ dj.get_datos(c,u,n) * x[u,k] for u in G.nodes() if u!=K] ), "Media_de_la_region_"+str(k) )
	mod.addConstr(grb.quicksum( [aux[u,k] for u in G.nodes() if u!=K])== grb.quicksum( [ dj.get_datos(c,u,n) * x[u,k] for u in G.nodes() if u!=K] ))
	mod.addConstr(media[k] <= 10000 * r[k])
	for u in G.nodes():
		if u!=K:
			mod.addConstr( aux[u,k] <= 10000 * x[u,k] )
			mod.addConstr( aux[u,k] >= media[k] -10000*(1- x[u,k]) )
			mod.addConstr( aux[u,k] <= media[k] )
